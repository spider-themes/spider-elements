!function($, $ele, $i) {
    window.liteTranslated = function (e, t) {
        return elementorCommon.translate(e, null, t, docyEditor.i18n)
    };
    ...
    $app.Behaviors.InsertTemplate = Marionette.Behavior.extend({
        ui: {
            insertButton: ".docyTemplateLibrary_insert-button"
        },
        events: {
    ...
    $app.Views.EmptyTemplateCollection = Marionette.ItemView.extend({
        ...
        template: "#docy-liteTemplateLibrary_empty",
        ui: {
    ...
    $app.Views.Loading = Marionette.ItemView.extend({
        template: "#docy-liteTemplateLibrary_loading",
        id: "liteTemplateLibrary_loading"
    }), 
    $app.Views.Logo = Marionette.ItemView.extend({
        template: "#docy-liteTemplateLibrary_header-logo",
    ...
    $app.Views.BackButton = Marionette.ItemView.extend({
        template: "#docy-liteTemplateLibrary_header-back",
    ...
    $app.Views.Menu = Marionette.ItemView.extend({
        template: "#docy-TemplateLibrary_header-menu",
    ...
    $app.Views.ResponsiveMenu = Marionette.ItemView.extend({
        template: "#docy-TemplateLibrary_header-menu-responsive",
    ...
    $app.Views.Actions = Marionette.ItemView.extend({
        template: "#docy-TemplateLibrary_header-actions",
    ...
    $app.Views.InsertWrapper = Marionette.ItemView.extend({
        template: "#docy-TemplateLibrary_header-insert",
    ...
    $app.Views.Preview = Marionette.ItemView.extend({
        template: "#docy-TemplateLibrary_preview",
    ...
    $app.Views.TemplateCollection = Marionette.CompositeView.extend({
        template: "#docy_TemplateLibrary_templates",
    ...
    $app.Views.Template = Marionette.ItemView.extend({
        template: "#docy-TemplateLibrary_template",
    ...
    $app.Modal = elementorModules.common.views.modal.Layout.extend({
        ...
        getTemplateActionButton: function(e) {
            var t = e.isPro && !docyEditor.hasPro ? "pro-button" : "insert-button";
            return viewId = "#docy-TemplateLibrary_" + t, template = Marionette.TemplateCache.get(viewId), Marionette.Renderer.render(template)
        },
        showLogo: function(e) {
    ...
    $app.Manager = function() {
        ...
        function addButton() {
            ...
            }), $sl.find(".docy_templates_add_button").length || $sl.find(DOCY_FIND_SELECTOR).before($docyPpenLibraryButton)
        }

        function initAddButton($el) {
            var $t = $el.find(DOCY_FIND_SELECTOR);
            $t.length && !$el.find(".docy_templates_add_button").length && $t.before($docyPpenLibraryButton), $el.on("click.onAddElement", ".elementor-editor-section-settings .elementor-editor-element-add", addButton)
        }

        function onChanges(t, i) {
            i.addClass("elementor-active").siblings().removeClass("elementor-active");
            var a = $docyResponsiveMap[t] || $docyResponsiveMap.desktop;
        ...
        function onEditorLoaded() {
            var $el = window.elementor.$previewContents,
                ...
                $el.on("click.onAddTemplateButton", ".docy_templates_add_button", $this.showModal.bind($this)), this.channels.tabs.on("change:device", onChanges)
        }

        var o, s, d, c, m, $this = this;
        console.log( window );

        DOCY_FIND_SELECTOR = ".elementor-add-new-section .elementor-add-section-drag-title",
        $docyPpenLibraryButton = '<div class="elementor-add-section-area-button docy_templates_add_button"> <img src="'+docyEditor.templateLogo+'" /> </div>',
        $docyResponsiveMap = {
            desktop: "100%",
            tab: "768px",
            mobile: "360px"
        ...
        this.init = function() {
            $this.setFilter("type", docyEditor.default_tab, !0), $ele.on("preview:loaded", onEditorLoaded.bind(this))
        }, 
        this.getTabs = function() {
            var e = this.getFilter("type");
            return tabs = JSON.parse(docyEditor.tab_style)
        ...
        this.requestLibraryData = function(e) {
            ...
            e.forceSync && (t.data.sync = !0), elementorCommon.ajax.addRequest("get_docy_library_data", t)
        }, 
        this.requestTemplateData = function(e, t) {
            ...
            t && jQuery.extend(!0, i, t), elementorCommon.ajax.addRequest("get_docy_template_data", i)
        }, 
...
}(jQuery, window.elementor, window.docy || {});
