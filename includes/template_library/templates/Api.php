...
class Api extends Source_Base {
	...
	const LIBRARY_OPTION_KEY = 'docy_library_info';

	const LIBRARY_TIMESTAMP_CACHE_KEY = 'docy_remote_update_timestamp';
	const API_INFO_URL = 'https://wordpress-theme.spider-themes.net/elementor-template-library/docy/';
    ...
    {
		return esc_html__( 'Docy Library', 'spider-elements' );
	}
	...
	private static function request_library_data( $force_update = false ): void
    {
    ...
    {
		return new \WP_Error( 'invalid_request', 'Cannot save template to a docy elementor addons library' );
	}
	
	public function update_item( $new_data ): \WP_Error
    {
		return new \WP_Error( 'invalid_request', 'Cannot update template to a docy elementor addons library' );
	}

	
	public function delete_template( $template_id ): \WP_Error
    {
		return new \WP_Error( 'invalid_request', 'Cannot delete template from a docy elementor addons library' );
	}

	public function export_template( $template_id ): \WP_Error
    {
		return new \WP_Error( 'invalid_request', 'Cannot export template from a docy elementor addons library' );
	}
	
	public static function request_template_data( $template_id ): string
    {
	...
	public function get_data( array $args, $context = 'display' ) {
		...
		if ( empty( $data ) || empty( $data['content'] ) ) {
			throw new \Exception( __( 'Template does not have any content', 'spider-elements' ) );
		}
