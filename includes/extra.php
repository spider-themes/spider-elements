...
function spe_get_the_excerpt_length( $settings, $settings_key, $default = 10 ) {
	...
	$excerpt        = get_the_excerpt() ? wp_trim_words( get_the_excerpt(), $excerpt_length,
...
function spe_posted_by() {
	...
	/* translators: %s: post author. */
		...
		'<span class="author"><a class="url fn n" href="' . esc_url( get_author_posts_url( $post->post_author ) ) . '">' . esc_html( get_the_author_meta( 'display_name',
			$post->post_author ) ) . '</a></span>'
	);
