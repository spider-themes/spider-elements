...
/**
 * Check if the current theme is Docy or if premium is active.
 *
 * @return bool
 */
 ...
 * @param bool  $is_echo      Whether to echo or return (currently only echo supported).
 */
...
if ( ! function_exists( 'spel_readable_number' ) ) {
	function spel_readable_number( $size ): int {
		...
		if ( isset( $exponents[ $suffix ] ) ) {
			$value *= pow( 1024, $exponents[ $suffix ] );
		}
